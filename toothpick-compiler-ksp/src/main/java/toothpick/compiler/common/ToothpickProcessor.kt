...
abstract class ToothpickProcessor : AbstractProcessor() {
    ...
    protected fun ExecutableElement.getExceptionTypes(): List<TypeElement> {
        val exceptionClassNames: MutableList<TypeElement> = mutableListOf()
        for (thrownTypeMirror in thrownTypes) {
    ...
    protected fun TypeElement.isExcludedByFilters(): Boolean {
        ...
        return options.excludes.any { exclude ->
            val regEx = exclude.toRegex()
            typeElementName.matches(regEx)
        }.also { isExcluded ->
            if (isExcluded) {
                warning(
                    this,
